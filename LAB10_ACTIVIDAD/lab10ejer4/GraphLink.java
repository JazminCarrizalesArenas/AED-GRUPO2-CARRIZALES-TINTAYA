...
public class GraphLink<E> implements Graph<E, Edge<E>> {
    ...
    //ejercicio 4:
    public void insertEdgeDirected(E origin, E destination) {
    ...
    public boolean isPlano() {
        ...
        for (int i = 0;i < graph.size();i++) {
            ...
            aristas += graph.get(i).getEdges().size();
        }
    ...
    public boolean isAutoComplementario() {
        ...
        int maximo =vertices *  (vertices - 1);
    
        return aristas ==maximo / 2;
    }

    public boolean isIsomorfo( GraphLink<E> otro) {
    
        if (this.graph.size() !=otro.graph.size()) {
            return false;
        }
