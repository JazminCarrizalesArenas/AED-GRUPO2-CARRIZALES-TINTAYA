class AVLTree<E extends Comparable<E>> extends LinkedBST<E> {
    ...
    private NodeAVL<E> deleteAVL(NodeAVL<E> node, E data) {
        ...
        // =============================================
            ...
            System.out.println("Nodo encontrado: "    + node.data);
            // HOJA
            ...
            // SOLO HIJO DERECHO
            if (node.left == null) {
                System.out.println("Caso BST: Un hijo derecho" );
                return (NodeAVL<E>) node.right;
            }

            // SOLO HIJO IZQUIERDO
            if (node.right == null) {
                System.out.println( "Caso BST: Un hijo izquierdo");
                return (NodeAVL<E>) node.left;
            }

            // DOS HIJOS
            NodeAVL<E> sucesor =getMin( (NodeAVL<E>) node.right );
            System.out.println("Caso BST: Dos hijos");

            System.out.println("Sucesor usado: "+ sucesor.data);
            node.data = sucesor.data;
            node.right =deleteAVL((NodeAVL<E>) node.right,sucesor.data );
        }

        // =============================================
        // ACTUALIZAR BF
        // =============================================
        node.bf =altura((NodeAVL<E>) node.right)-altura((NodeAVL<E>) node.left);
        // =============================================
        // REBALANCEAR
        // =============================================
        if (node.bf < -1) {
            System.out.println("Nodo desbalanceado: " + node.data);
    ...
    private int altura(NodeAVL<E> node) {
        ...
        int izq =altura((NodeAVL<E>) node.left);
        int der =altura((NodeAVL<E>) node.right);
